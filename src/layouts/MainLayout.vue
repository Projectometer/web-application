<template>
    <q-layout class="o-layout" view="hHh LpR fFf">
        <div :class="!isLoggedIn ? 'o-container' : ''">
            <!-- Be sure to play with the Layout demo on docs -->
            <q-header class="row q-layout-header" v-if="!$q.platform.is.mobile">
                <q-toolbar class="q-px-md q-py-sm">
                    <div class="row col col-md-2 col-sm-3 col-xs-12">
                    ...
                    <ToolbarComponent
                        class="col col-md-10 col-sm-9 col-xs-12"
                    ></ToolbarComponent>
                </q-toolbar>
                <q-toolbar class="q-px-sm" v-if="isLoggedIn">
                    <div class="q-px-md full-width">
                        <div
                            class="height-60 bg-black rounded-borders-10 q-mx-none q-my-lg clendar-horizontal-hover full-width"
                        >
                            <DayWeekToolbarComponent
                                @on-select-date="onSelectDate"
                                ref="dayWeekToolbarComponent"
                            ></DayWeekToolbarComponent>
                        </div>
                    </div>
                </q-toolbar>
            ...
            <q-header
                class="row q-layout-header-mobile"
                v-if="$q.platform.is.mobile"
            >
                <q-toolbar class="q-px-xs q-py-sm items-center">
                    <div class="row col col-md-2 col-sm-3 col-xs-4">
                        <q-btn
                            icon="menu"
                            flat
                    ...
                    <div class="row col col-md-2 col-sm-3 col-xs-8">
                        <q-avatar class="avatar-logo" rounded size="2.5rem">
                            <svg
                                style="
                                    shape-rendering: geometricPrecision;
                    ...
                    <q-drawer side="left" v-model="drawerLeft" class="o-layout">
                        <MainMenu></MainMenu>
                    </q-drawer>
                    <!-- <ToolbarComponent
                        class="col col-md-10 col-sm-9 col-xs-12"
                ...
                <q-toolbar v-if="isLoggedIn" class="q-px-none m-toolbar">
                    <q-list
                        class="rounded-borders items-center bg-white full-width o-layout my-mobile"
                    >
                        <q-expansion-item class="o-layout">
                            <div class="q-px-sm full-width">
                                <div
                                    class="height-60 bg-black rounded-borders-10 q-mx-none q-my-lg clendar-horizontal-hover full-width"
                                >
                                    <DayWeekToolbarMobileComponent
                                        @on-select-date="onSelectDate"
                                        ref="dayWeekToolbarComponent"
                                    ></DayWeekToolbarMobileComponent>
                                </div>
...
import DayWeekToolbarComponent from 'src/components/DayWeekToolbarComponent.vue';
import DayWeekToolbarMobileComponent from 'src/components/DayWeekToolbarMobileComponent.vue';
import MainMenu from 'src/components/common/MainMenu.vue';
import { today } from '@quasar/quasar-ui-qcalendar';
const dayWeekToolbarComponent = ref();
...
const onSelectDate = function (date: string) {
    if (date) {
        selectedDate.value = date;
        dayWeekToolbarComponent.value?.setDate(date);
    }
};
</script>
